...
struct CreepType
{
  ...
  CreepType(uint8_t sprite = 0, uint8_t hp = 1, uint8_t speed = 1, uint8_t reward = 1) : mSprite(sprite), mMaxHealth(hp), mSpeed(speed), mReward(reward)
  {
  }
};
...
struct Tile
{
  ...
  Tile(TileType type = TILE_EMPTY, TowerInstance tower = TowerInstance(0)) : mType(type), mTower(tower)
  {
  }
...
protected:
  ...
  {
    levelFrom = min(CREEPS_TOTAL - 1, int(levelFrom));
    levelTo = min(CREEPS_TOTAL - 1, int(levelTo));
  ...
  {
    total = min(MAX_CREEPS, int(total));
    ...
    for (uint8_t i = 0; i < total; ++i)
    {
      ...
      CreepInstance c = CreepInstance(
          ...
          position,
          i % groupSize == 0 ? min(3, int(maxLives)) : 0);
...
public:
                           ...
                           CreepInstance *creepArray, uint16_t *creepCount) override
  {
    cyclingSpawn(
        ...
        roundNumber < 15 ? min(CREEP_DINO - 1, int(roundNumber)) : roundNumber, // l to
...
struct Game
{
  ...
  void buttonDown(uint8_t button)
  {
    ...
    case STATE_MENU:
      ...
      case LEFT_BUTTON:
        mSelectedMenuItem =
            mSelectedMenuItem == 0 ? (MAIN_MENU_ITEMS - 1) : mSelectedMenuItem - 1;
        break;
...
void drawMap()
{
  ...
  if (game.mState == STATE_PLAYING_MENU || !arduboy.pressed(B_BUTTON))
  {
    ...
    uint8_t y = (game.mState != STATE_PLAYING_MENU && game.mCursor.mY < 1) ? DISPLAY_HEIGHT - INFOBAR_Y - 7 : INFOBAR_Y;
